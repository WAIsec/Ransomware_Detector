...
def print_info(data_list):
    for data in data_list:
        print(data[0].ljust(20), str(data[1]).ljust(20), data[2].ljust(20)) # ljust = 왼쪽 정렬을 위한 method

# dos_header에 대한 파싱 함수        
def dos_header_info (pe):
    print("-"*30)
    print("[DOS Header]에서 필요한 정보\n")
    dos_header_list = []
    dos_header_list.append(["Name", "Value", "Present"])
...
def nt_header_info (pe):
    ...
    nt_header_list = []
    nt_header_list.append(["Name", "Value", "Present"])
    ...
    nt_header_list.append(["AddressOfEntryPoint", hex(pe.OPTIONAL_HEADER.AddressOfEntryPoint), "PE 파일이 메모리 로드 후 처음 실행 되어야 하는 코드 주소"])
    nt_header_list.append(["ImageBase", hex(pe.OPTIONAL_HEADER.ImageBase), "PE파일이 매핑되는 시작주소"])
    nt_header_list.append(["SectionAlignment", pe.OPTIONAL_HEADER.SectionAlignment, "메모리 상에서으 ㅣ최소 섹션 단위"])
...
def sections_header_info(pe):
    ...
    print("Charateristics".ljust(20), "섹션의 특징을 나타냄")
    ...
    print("Name".ljust(20), "VirtualAddress".ljust(20), "SizeOfRawData".ljust(20), "PointerToRawData".ljust(20), "Characteristics".ljust(20))
    for section in pe.sections:
        print(section.Name.decode('utf8').ljust(20), hex(section.VirtualAddress).ljust(20), hex(section.SizeOfRawData).ljust(20), hex(section.PointerToRawData).ljust(20), hex(section.Characteristics))
    
    print("-" * 30,"\n")

# main
def main():
    print("파싱을 원하는 PE 파일 경로 입력")
    path = input("=>")
    path = path.replace('"',"")
    pe = pefile.PE(path)
    print("Parsing...")
